...
## Warm-up activity: Portraits! (30 minutes)

We're going to paint portraits of each other. Let's all use a canvas size of 200 by 200 pixels. 

You can use any shape, color. And you may want to use a custom shape (especially for elements like hair).

Commands that may be useful:

```
background(r, g, b); //resets background color each time it runs
fill(r, g, b);       //sets color 
rect(x, y, w, h);    //x, y is the location of top left
ellipse(x, y, w, h); //x, y is location of the center
triangle(x1, y1, x2, y2, x3, y3);
line(x1, y1, x2, y2);
```

## Review portraits and recreations of famous artworks (20 minutes)


## Flash Presentations! (45 minutes)
...
![variables]({{"assets/img/week2/variable.jpg" }} )
...
In some languages, you have to specify what kind of variable you want depending on what you want it to hold, like a decimal, a word, a whole number, or something else. In p5.js (and Javascript), you only have to say ```let``` to show you want a variable and follow it with the name of the variable you would like. **Important:** You will also sometimes see people use ```var``` instead of ```let```. Programming languages evolve over time. This was the older version. Either are fine to use in our class.

```
let age;
```
<iframe src="https://giphy.com/embed/Cglm3JaOZFSOFYx1qY" width="480" height="480" frameBorder="0" class="giphy-embed" allowFullScreen></iframe>
...
<iframe src="https://giphy.com/embed/cFv3zac2OkvIak9gt7" width="480" height="270" frameBorder="0" class="giphy-embed" allowFullScreen></iframe>
...
![Locals only]({{"assets/img/week2/locals-only.png" }} )
...
<img style="width:350px;height:500px;" src='{{"assets/img/week2/conditions.jpg" }}'>
...
Revisit your code from the portrait activity.

Add interactivity and randomness.

Start by duplicating (file > duplicate) your previous code sketch. 

Now begin by creating global variables for elements of your sketch. You can create variables for x, y locations, colors, widths, etc. Use variable names that are descriptive. Examples:
...
Use at least 10 variables, though you can use much more if you like. Remember to use a sketch size of 200 by 200 again.
